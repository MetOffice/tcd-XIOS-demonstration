...
!>   Here sim_period = 24 and sim_start_offset = 16, the latter indicating the number of time
!>   steps since original sim start date (2022-12-13-13-0-0) to the first time instant in
!    the relevant input split file.
!> - The adjusted simulation start date/time here is: 2022-12-14-05-0-0 which is sim_start_offset = 16
...
!>   different from the original sim settings - i.e. by removing the first sim_start_offset = 16
...
contains
  ...
  subroutine initialise_xios()
    ...
    start = xios_date(2022, 12, 14, 05, 0, 0)
    tstep = xios_hour
  ...
  subroutine simulate()
    ...
    integer, parameter :: sim_start_offset = 16
